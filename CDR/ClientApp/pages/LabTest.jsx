...
export default class LabTest extends React.Component 
{
    ...
    async selectState(value)
    {
        ...
        this.setState({ data: data, sites: sites, labTests: [] });
    }
    ...
    async export(state)
    {
        ...
        let header = "TX_CURR Clients' Laboratory Results";
        if (state.site.id && state.site.id !== '' && parseInt(state.site.id) > 0)
        {
            header = 'TX_CURR Laboratory Results for ' + state.site.name; 
        }
        if (state.id && state.id !== '' && parseInt(state.id) > 0)
        {
            header += ', ' + state.name;
        }
        ...
        let res = await postQuery('/api/Stats/exportLabTests', s);
    ...
    render() 
    {        
        const { sites, data, loading, statesLoaded, states, labTests } = this.state;
        let d = data;
        if (!statesLoaded)
        {
            ...
            <div style={{marginTop: '15px', padding: '20px'}}>
                ...
                <div className="custom-filter-dropdown">
                    <Row style={{ marginTop: '2px' }}>
                        <Col span={24}>
                            <h4 style={{fontWeight: 'bold', fontSize: '18px'}}>TX_CURR Laboratory Results </h4>
                        </Col>
                    </Row>
                    <br />
                    <Row>
                            ...
                            <Row key={d.id} style={{ marginTop: '20px', borderBottom: 'dotted thin', marginBottom: '40px' }}>
                                <Row style={{ display: d.getLineList ? 'none' : 'block' }}>
                                    ...
                                    <Col xs={3} sm={3} md={3} lg={3} style={{ marginLeft: '15px' }}>
                                        <Button style={{ marginBottom: '8px' }} icon="ordered-list" size="large" disabled={loading} className="float-left" onClick={() => el.filter(d)}>Get Results</Button>
                                    </Col>
                                    <Col xs={3} sm={3} md={3} lg={3} style={{ marginLeft: '15px' }}>
                                        {labTests.length > 0 && <Button disabled={loading} icon={loading ? '' : "download"} onClick={() => this.export(d)} style={{ float: 'right' }}>
                                            {loading ? 'Please wait...' : 'Export to Excel'}
                                        </Button>}
                                    </Col>
                                    <br />
                                    <Table columns={columns} rowKey={record => record.id} dataSource={labTests} pagination={false} loading={loading} bordered type="flex" align="middle" />
                                    <br />
                                </Row>
